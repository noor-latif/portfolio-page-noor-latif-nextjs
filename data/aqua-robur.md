# Aqua Robur Technologies - Hardware and Software Developer

**Duration:** February 2021 - September 2022 (1 year 8 months)
**Location:** Gothenburg, Västra Götaland County, Sweden

## Company Background
Aqua Robur Technologies was recognized on '33-listan' for Sweden's best startups in innovation and growth. The company focuses on innovative IoT solutions for industrial applications.

## Overview
Developed embedded IoT solutions and provisioning automation at a high-growth startup, working across the full stack from embedded firmware to industrial automation systems.

## Key Achievements

### OPC-UA Infrastructure Management
- Managed OPC-UA server infrastructure connecting client SCADA/HMI systems
- Automated device connections via KepServerEX API
- Implemented reliable communication protocols for industrial IoT devices
- Ensured data integrity and real-time monitoring capabilities
- Configured tag mappings and data transformations for various client systems

### Python Automation (60% Efficiency Gain)
- Automated device configuration processes with Python, increasing production efficiency by 60%
- Eliminated manual errors in provisioning workflows
- Developed scripts for bulk device configuration and testing
- Created automated validation processes to ensure device quality
- Implemented logging and monitoring for production processes
- Reduced human intervention in repetitive configuration tasks

### Embedded Systems Development
- Developed portable sensor-tester with built-in display for manufacturing
- Created stress testing procedures for level sensors with live results
- Enabled real-time troubleshooting during manufacturing process
- Designed hardware-software integration for testing equipment
- Implemented display interface for immediate feedback to operators

### Firmware Optimization (30% Energy Improvement)
- Optimized turbine energy harvester firmware in embedded C/C++
- Achieved 30% improvement in energy efficiency through algorithmic improvements
- Analyzed power consumption patterns and identified optimization opportunities
- Implemented low-power modes and efficient data processing
- Balanced performance requirements with energy constraints
- Validated improvements through extensive testing and measurement

### Security & Certificate Management
- Developed certificate upgrade processes for industrial systems
- Strengthened security protocols for IoT device communication
- Implemented secure boot and firmware verification processes
- Ensured compliance with industrial security standards

### Mentorship & Knowledge Sharing
- Mentored Chalmers University master's thesis students
- Provided guidance on sensor development and IoT engineering practices
- Shared industry best practices and real-world problem-solving approaches
- Contributed to academic-industry collaboration

## Technical Stack
- **Industrial Protocols:** OPC-UA, KepServerEX
- **Programming Languages:** Python (automation), Embedded C/C++ (firmware)
- **IoT Technologies:** Sensor integration, industrial IoT systems
- **SCADA/HMI:** Client system integration and configuration
- **Hardware:** Sensor development, testing equipment design
- **Security:** Certificate management, secure communication protocols

## Impact Metrics
- **Production Efficiency:** 60% increase through automation
- **Energy Efficiency:** 30% improvement in firmware optimization
- **Error Reduction:** Eliminated manual configuration errors
- **Quality Assurance:** Real-time testing capabilities during manufacturing

## Technical Challenges Solved
1. **Scalability:** Automated provisioning to handle growing device fleet
2. **Reliability:** Ensured robust communication in industrial environments
3. **Energy Constraints:** Optimized firmware for energy harvesting applications
4. **Integration Complexity:** Connected diverse SCADA/HMI systems via OPC-UA
5. **Manufacturing Efficiency:** Created tools to accelerate production testing

## Innovation Highlights
- Portable sensor testing solution for manufacturing floor
- Automated device provisioning pipeline
- Energy-efficient firmware algorithms for resource-constrained devices
- Secure certificate management for industrial IoT
